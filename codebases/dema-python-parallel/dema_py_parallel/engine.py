...
try:
    ...
    def set_num_threads(_n: int):  # type: ignore
        return None
...
class ParallelDemaEngine:
    ...
    def run(self) -> LayoutResult:
        if HAS_NUMBA:
            # Use Numba threading backend as the primary parallel execution path.
            effective_workers = self._effective_workers()
            set_num_threads(effective_workers)
            self._effective_workers_used = effective_workers
            rounds_completed, final_energy = _run_numba_parallel(
                self.positions,
                self.adjacency,
                self.weight,
                self.fc,
    ...
    @property
    def effective_workers_used(self) -> int:
        return self._effective_workers_used
    ...
    def _run_numpy_fallback(self) -> Tuple[int, float]:
        ...
        for round_idx in range(self.params.max_rounds):
            ...
            updates = np.zeros((self.n, 2), dtype=np.float64)
            for node in range(self.n):
                ...
                updates[node, 0] = grad[0] * step
                updates[node, 1] = grad[1] * step
